...
import net.heishanrm.M3u8.utils.Log;
import net.heishanrm.M3u8.utils.MediaFormat;
import net.heishanrm.M3u8.utils.StringUtils;
...
public class DownloadFactory {
    ...
    private static volatile M3u8Download m3u8Download;
            ...
            new Thread(() -> {
                ...
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        Log.i(new BigDecimal(finishedCount).divide(new BigDecimal(tsSet.size()), 4, RoundingMode.HALF_UP).multiply(new BigDecimal(100)).setScale(2, RoundingMode.HALF_UP) + "%");
            ...
            new Thread(() -> {
                ...
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        for (DownloadListener downloadListener : listenerSet)
                            downloadListener.process(DOWNLOADURL, finishedCount, tsSet.size(), new BigDecimal(finishedCount).divide(new BigDecimal(tsSet.size()), 4, RoundingMode.HALF_UP).multiply(new BigDecimal(100)).setScale(2, RoundingMode.HALF_UP).floatValue());
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                }
